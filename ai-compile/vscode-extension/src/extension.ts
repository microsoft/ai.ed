...
export function activate( context: vscode.ExtensionContext ) {
	...
	disposables.push( vscode.commands.registerCommand( 'python-hints.toggleHints', async function () {
		if( c.DEBUG ) {
			console.log( "toggleFix Command triggered..." );
		}
		const flag = vscode.workspace.getConfiguration( "python-hints" ).get( "enableCodeLens", false );
		vscode.workspace.getConfiguration( "python-hints" ).update( "enableCodeLens", !flag, true );
		...
		if( activeEditor !== undefined ) {
			...
			} else { // document.save() doesn't trigger if document isn't dirty
	...
	disposables.push( vscode.commands.registerCommand( 'python-hints.toggleHighlight', async function () {
		const flag = vscode.workspace.getConfiguration( "python-hints" ).get( "activeHighlight", false );
		vscode.workspace.getConfiguration( "python-hints" ).update( "activeHighlight", !flag, true );
